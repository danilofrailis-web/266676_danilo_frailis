...
class Model(nn.Module):
    def __init__(
        ...
        self.pos_embed = nn.Embedding(pos_embed_size, d_model)
    ...
    def forward(self, idx):
        ...
        #pos_embed_size is the maximum processable sequence length
        assert L <= self.pos_embed_size
...
class MultiHeadAttention (nn.Module):
    ...
    def forward(self, x, mask):
        ...
        #(B, n_heads, L, L) * (B, n_heads, L, h_dim) = (B, n_heads, L, h_dim)
        y = attn @ v
        #(B, L, n_heads, h_dim)
        y = y.transpose(1, 2)
        #(B, L, d_model)
